...
export const snippetPlugin = (md: MarkdownIt, srcDir: string) => {
  const parser: RuleBlock = (state, startLine, endLine, silent) => {
    ...
    const [filename = '', extension = '', region = '', lines = '', lang = '', rawTitle = ''] = (
      rawPathRegexp.exec(rawPath) || []
    ).slice(1);

    // Reading actuall code snippet from file
    const customSrc = path.resolve(filename) + region;
    ...
    if (isAFile) {
      ...
      if (regionName) {
        ...
        if (region) {
          ...
          if (partialPath) {
            url = 'https://github.com/FuelLabs/fuels-ts/blob/master/'
              .concat(`${partialPath}`)
              .concat(`#L${start}-L${++end}`);
          }
        }
      }
      ...
      Object.assign(token, {
        ...
        content,
        customSrc: path.resolve(filename) + region,
        markup: '```',
        map: [startLine, startLine + 1],
        info: `${lang || extension}${lines ? `{${lines}}` : ''}${title ? `[${title}]` : ''}`,
      });
    }
